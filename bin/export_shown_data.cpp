		...
		if( all.size() ){
			...
			for( int i=0 ; i<all.size() ; ){
				...
				for( int j=last_sample_time+1 ; j<sample_time_range ; j++ ){
					...
					cout << to_iso_extended_string(from_time_t(j*range_selector))<< "+00:00";
					for( int k=0 ; k<channels.size() ; k++ ){
						if( k ){
							//chartData += "," + conv(channels[k].value , 4);
							cout << "," << conv(channels[k].value , 4);
						}
						else
							//chartData += conv(channels[k].value, 4);
							cout << "#" << conv(channels[k].value, 4);
				...
				cout << to_iso_extended_string(from_time_t(sample_time_range*range_selector))<< "+00:00";
				for( int k=0 ; k<channels.size() ; k++ ){
					if( k ){
						//chartData += "," + conv(channels[k].value , 4);
						cout << "," << conv(channels[k].value , 4);
